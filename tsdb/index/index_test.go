...
func TestPostingsMany(t *testing.T) {
	...
	for i := 1; i < 1000; i++ {
		v := fmt.Sprintf("%03d", i)
		series = append(series, labels.FromStrings("i", v))
		symbols[v] = struct{}{}
	}
	symbols["i"] = struct{}{}
...
func TestPersistence_index_e2e(t *testing.T) {
	...
	for p := range mi.postings {
		gotp, err := ir.Postings(p.Name, []string{p.Value})
		testutil.Ok(t, err)

		expp, err := mi.Postings(p.Name, []string{p.Value})
		testutil.Ok(t, err)
